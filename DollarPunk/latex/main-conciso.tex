...
Poiché lo spazio è noto solo attraverso un numero finito di embedding, si usa l'approssimazione discreta canonica di $-\nabla^2$, il \emph{Laplaciano di grafo} $L=\operatorname{diag}(d_i)-W$, con $n$ ticker come nodi e pesi $w_{ij}\ge 0$ dati dalla similarità coseno fra embedding (sogliata, $W$ simmetrica). La PDE diventa un sistema lineare di ODE,
\[
