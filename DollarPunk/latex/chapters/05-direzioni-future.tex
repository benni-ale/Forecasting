...
\subsection{Diffusione semantica su grafo}
\label{sec:soluzione-discreta}

L'idea del Capitolo~\ref{ch:descrizione-progetto}---una rilevanza che decade nel tempo e si propaga tra titoli affini---si formalizza su un grafo di similarità: i nodi sono gli $n$ ticker, i pesi $w_{ij} \geq 0$ la similarità coseno tra gli embedding aziendali (sogliata, con $W$ simmetrica), $d_i = \sum_j w_{ij}$ i gradi e $L = \operatorname{diag}(d_i) - W$ il Laplaciano del grafo.
La rilevanza $R(t) \in \mathbb{R}^n$ evolve secondo
\[
\frac{dR}{dt} = -(D\,L + \lambda I)\, R,
\]
dove $\lambda > 0$ è il tasso di decadimento già in uso e $D \geq 0$ regola quanta rilevanza fluisce verso i titoli vicini.
Per il sentiment basta una accortezza: insieme a $R$ si fa evolvere, con lo stesso operatore, il \emph{sentiment pesato} $V = S \cdot R$ (la stessa quantità $s \cdot r$ già usata nel KPI), e si definisce $S_i(t) = V_i(t)/R_i(t)$.
Così la rilevanza che si sposta da un titolo all'altro porta con sé il proprio tono, invece di gonfiare quello locale.
Rispetto a una GNN, i due parametri $D$ e $\lambda$ restano fisicamente leggibili: il confronto tra questa diffusione interpretabile e un baseline appreso costituirebbe il contributo scientifico centrale del progetto.
