\chapter{Direzioni future}
\label{ch:direzioni-future}

Questo capitolo delinea le direzioni di sviluppo più promettenti, scientifiche e applicative: l'applicativo fornisce l'infrastruttura di dati e validazione su cui le domande di ricerca possono essere testate empiricamente.

\section{Direzioni per la ricerca}

\subsection{Stima empirica del parametro di decay}

Nel prototipo l'emivita del relevance score è configurabile (default: 7 giorni) ma non stimata dai dati.
Una prima domanda a basso costo sperimentale è la stima ottimale di $h$, selezionando il valore che massimizza la correlazione ritardata tra KPI di sentiment e rendimenti successivi, per titolo o settore.
L'ipotesi verificabile è che l'emivita informativa non sia costante, ma vari con capitalizzazione, copertura mediatica e regime di mercato---un risultato autonomo rispetto al decay fisso dei provider commerciali.

\subsection{Diffusione semantica: dalla PDE alla soluzione discreta}
\label{sec:soluzione-discreta}

\paragraph{Il modello proposto.}
Si rappresenti ogni contenuto informativo come un punto $x$ di uno spazio semantico (lo spazio degli embedding), e sia $R(x,t)$ la rilevanza attiva in quel punto al tempo $t$.
Il modello proposto estende il decadimento del Capitolo~\ref{ch:descrizione-progetto} con un termine di diffusione:
\[
\frac{\partial R(x, t)}{\partial t} = \underbrace{D\, \nabla^2 R(x, t)}_{\text{propagazione}} \; - \; \underbrace{\lambda\, R(x, t)}_{\text{decadimento}}.
\]

\paragraph{Perché ha senso.}
È l'equazione del calore con dissipazione, e l'analogia è sostanziale, non estetica.
Il secondo termine è il modello già validato in produzione: in assenza di interazioni, la rilevanza di una notizia si dimezza ogni $h = \ln 2/\lambda$ giorni.
Il primo termine cattura un fatto empirico che il decadimento da solo ignora: l'informazione rilevante per un titolo lo è, in parte, anche per i titoli semanticamente vicini---una notizia sul principale fornitore di un'azienda ne muove il titolo, un downgrade di settore pesa su tutto il settore.
Come il calore fluisce da un punto caldo ai punti freddi adiacenti in proporzione alla differenza di temperatura, così la rilevanza fluisce dai titoli molto coperti verso i titoli affini poco coperti, in proporzione al gradiente informativo e alla vicinanza semantica.
I due parametri hanno un significato fisico immediato---$\lambda$ è la velocità dell'oblio, $D$ la permeabilità semantica del mercato---ed è questa leggibilità a distinguere il modello dalle alternative apprese (GNN), i cui pesi non sono interpretabili: il confronto tra i due approcci costituirebbe il contributo scientifico centrale del progetto.

\paragraph{Soluzione discreta.}
Lo spazio semantico è però noto solo attraverso un numero finito di punti---gli embedding dei ticker---e su un insieme finito di punti l'operatore $-\nabla^2$ ha un'approssimazione discreta canonica: il Laplaciano di grafo.
Si costruisce quindi il grafo di similarità con gli $n$ ticker come nodi. Gli archi candidati collegano esclusivamente aziende appartenenti alla stessa industry; al suo interno i pesi $w_{ij} \geq 0$ sono dati dalla similarità coseno tra embedding (sogliata, $W$ simmetrica), mentre tra industry diverse $w_{ij}=0$. Le aziende prive di industry sono escluse. I gradi sono $d_i = \sum_j w_{ij}$ e $L = \operatorname{diag}(d_i) - W$. Un'anteprima tecnica del modello è disponibile su \url{https://dollarpunk-26786f215068.herokuapp.com/semantic-graph}.
La PDE diventa il sistema di equazioni ordinarie
\[
\frac{dR}{dt} = -(D\,L + \lambda I)\, R, \qquad R(t) \in \mathbb{R}^n.
\]
Per il sentiment basta un'accortezza: insieme a $R$ si fa evolvere, con lo stesso operatore, il \emph{sentiment pesato} $V = S \cdot R$ (la stessa quantità $s \cdot r$ già usata nel KPI), e si definisce $S_i(t) = V_i(t)/R_i(t)$.
Così la rilevanza che si sposta da un titolo all'altro porta con sé il proprio tono, invece di gonfiare quello locale.

\paragraph{Perché il modello si comporta bene.}
Riscritta nodo per nodo, l'equazione diventa
\[
\frac{dR_i}{dt} = D \sum_j w_{ij}\,(R_j - R_i) \; - \; \lambda R_i,
\]
e in questa forma le sue proprietà si leggono direttamente.
Il flusso tra due titoli è proporzionale alla \emph{differenza} di rilevanza: ciò che esce da un nodo entra esattamente nel vicino, quindi sommando su tutti i nodi i termini di diffusione si elidono a coppie e resta $\frac{d}{dt}\sum_i R_i = -\lambda \sum_i R_i$.
La rilevanza complessiva del mercato decade dunque \emph{esattamente come nel modello attuale}: la diffusione redistribuisce, non crea né distrugge.
Per lo stesso motivo la rilevanza non può diventare negativa: se $R_i$ si avvicina a zero, ogni differenza $R_j - R_i$ è non negativa e il flusso può solo rientrare.
Essendo il sistema lineare, la soluzione si scrive anche in forma chiusa, $R(t) = e^{-\lambda t}\, e^{-DtL} R(0)$, e per $D = 0$ le equazioni si disaccoppiano riducendosi al decadimento già in produzione ($w = 0{,}5^{\Delta t/h}$ con $h = \ln 2/\lambda$): il sistema attuale è il caso particolare senza diffusione.

Quanto al sentiment, poiché $V = S \cdot R$ evolve con lo stesso operatore di $R$, il sentiment di un titolo al tempo $t$ è il rapporto tra due somme costruite con gli stessi coefficienti non negativi---in altre parole, una \emph{media pesata dei sentiment iniziali}, dove il peso di ciascun titolo è la quota di rilevanza che da lì è arrivata.
E una media pesata non può mai superare il massimo né scendere sotto il minimo dei valori che media: la diffusione non può fabbricare un sentiment più estremo di quello effettivamente osservato nelle notizie.
È la garanzia che conta per un sistema che genera segnali operativi: nessuna amplificazione spuria.

In pratica, popolare \texttt{ticker\_cross\_diffused\_sentiment} richiede un aggiornamento al giorno di $(R, V)$---la soluzione di un piccolo sistema lineare sparso, stabile per qualunque passo temporale---con la matrice $W$ già disponibile dagli embedding.
Il caso $D = 0$ fornisce anche il test di validazione naturale: disattivando la diffusione, la nuova pipeline deve riprodurre esattamente i KPI attuali.

\subsection{Benchmark multi-scorer del sentiment}

La delega dello scoring ad Alpha Vantage può diventare un'opportunità: il database contiene decine di migliaia di coppie (articolo, ticker) già etichettate.
Si propone un confronto sistematico tra scorer sullo stesso corpus---FinBERT zero-shot, un LLM open-weight via prompting, i punteggi del provider---su due domande: la concordanza inter-rater e la validità predittiva rispetto ai rendimenti.
La conversione \textit{logit-to-score} di FinDPO \parencite{iacovides2025findpo} offre punteggi continui e calibrati, confrontabili con la scala $[-1, +1]$ in uso.

\subsection{Attenzione mediatica come segnale complementare}

Il volume di notizie per ticker e topic, già raccolto dalla coverage ingestion, permette di costruire un indice di attenzione mediatica nello spirito dell'Hype Index \parencite{cao2025hypeindexnlpdrivenmeasure} senza costi aggiuntivi.
L'ipotesi è che i picchi di copertura anticipino volatilità indipendentemente dalla polarità, e che l'interazione sentiment~$\times$~attenzione superi i due segnali separati.

\subsection{Interazione tra sentiment e segnali tecnici}

Con gli indicatori tecnici (Sezione~\ref{sec:analisi-tecnica}) il prototipo espone per ogni titolo un segnale informativo e uno di prezzo; la domanda naturale è se il sentiment abbia \emph{valore predittivo incrementale} rispetto ai segnali tecnici, e se le divergenze anticipino inversioni.
\textcite{neely2014forecasting} offrono il precedente diretto: gli indicatori tecnici prevedono il premio al rischio con potere comparabile e \emph{complementare} alle variabili macroeconomiche.
Il disegno proposto replica quello schema sostituendo alle variabili macro il sentiment: regressioni dei rendimenti su sentiment, segnale tecnico e interazione \parencite{brock1992simple}.
Il report automatico del Capitolo~\ref{ch:stato-implementazione} già classifica ogni titolo come allineato o divergente: le etichette costituiscono le celle del disegno sperimentale, a costo quasi nullo.

\subsection{Notizie programmate e non programmate}

\textcite{boudoukh2019information} mostrano che l'impatto delle notizie firm-specific differisce tra eventi \emph{programmati} (earnings, guidance) e \emph{non programmati} (M\&A, azioni legali).
La pipeline già classifica gli articoli per topic (inclusi \texttt{earnings} e \texttt{mergers\_and\_acquisitions}): separare il KPI nei due regimi permetterebbe di verificare se l'emivita informativa $h$ differisca tra notizie attese e sorprese, raffinando la stima empirica del decay proposta sopra---un'analisi resa possibile dallo storico pluriennale del cold store.

\subsection{Foundation model per serie temporali}

La proposta originaria individua nell'LSTM il modello predittivo di riferimento; dal 2024 i modelli fondazionali per serie temporali---Chronos \parencite{ansari2024chronos}, TimesFM \parencite{das2024timesfm}---raggiungono in zero-shot prestazioni comparabili ai modelli supervisionati per singolo dataset.
La revisione a basso costo del piano: usare un foundation model come baseline zero-shot e verificare se il condizionamento sul sentiment (covariata o fine-tuning leggero) apporti valore incrementale.
Il confronto ``LSTM da zero vs.\ foundation model condizionato sul sentiment'' è a sua volta una domanda di ricerca attuale.

\subsection{Estensione al mercato italiano}
\label{sec:mercato-italiano}

La pipeline a valle dell'ingestion---decay, KPI, diffusione, archivio, indicatori tecnici---è agnostica rispetto al mercato: estendere il sistema a Piazza Affari richiede di sostituire tre soli componenti.
Le \emph{fonti}: non esiste un provider che etichetti le news italiane, ma i feed RSS delle testate finanziarie e i comunicati price-sensitive (dove l'emittente coincide con l'entità, fornendo etichette di rilevanza esatte per costruzione) offrono materia prima abbondante.
I \emph{prezzi}: i titoli di Borsa Italiana sono coperti dai data provider gratuiti.
Lo \emph{scoring}, il nodo vero: la strategia proposta è il bootstrap con LLM-as-annotator su un sottoinsieme di news italiane, seguito dal fine-tuning di un modello multilingue sul mix tra il corpus inglese già etichettato del cold store e il sottoinsieme italiano annotato, con l'anagrafica degli alias societari (``Intesa'', ``Intesa Sanpaolo'', ``ISP'') come punto critico dell'entity linking.

Il primo passo è già operativo: un collector RSS (\texttt{italian\_news\_collector.py}) legge quotidianamente sei testate italiane con inserimento idempotente e \emph{senza} scoring. Un dizionario trasparente di nomi e alias delle società FTSE MIB filtra titolo e sintesi: per default vengono conservate soltanto le notizie che citano almeno un'azienda quotata; quelle economiche generali sono marcate come macro e possono essere raccolte separatamente. I ticker individuati sono candidati da verificare nella successiva fase di scoring, non etichette definitive. In questo modo il cold store accumula un corpus azionario più preciso senza contaminare i KPI correnti.
La prospettiva di ricerca è distintiva: la sentiment analysis finanziaria è quasi esclusivamente anglocentrica, e un dataset entity-level italiano validato sul FTSE~MIB---mercato con dinamiche di copertura mediatica diverse da quelle statunitensi---costituirebbe un contributo autonomo nel filone del financial NLP a basse risorse.

\subsection{Protocollo di backtesting metodologicamente rigoroso}

La letteratura critica \parencite{li2025finsaber} mostra che i vantaggi delle strategie LLM svaniscono su orizzonti lunghi e universi ampi, e che i backtest sono esposti a survivorship bias, look-ahead bias e data-snooping.
Il backtesting sarà quindi vincolato a un protocollo esplicito: solo informazione disponibile alla data di segnale, benchmark passivi, più regimi di mercato, costi di transazione dichiarati.
Per gli scorer LLM va considerato anche il look-ahead bias ``parametrico'': la valutazione pulita richiede notizie successive al cutoff di addestramento del modello.

\section{Direzioni per l'applicativo}

Le priorità implementative, per rapporto valore/sforzo:

\begin{enumerate}
  \item \textbf{Scoring interno con FinBERT}: applicazione batch agli articoli in database, con punteggi persistiti accanto a quelli del provider---abilita il benchmark multi-scorer;
  \item \textbf{Popolamento del cross-diffused sentiment} con il modello della Sezione~\ref{sec:soluzione-discreta} e la matrice di correlazione esistente;
  \item \textbf{Analisi statistica sentiment--prezzo}: correlazioni di Pearson e Spearman con ritardi configurabili ($0$--$5$ giorni), al posto del confronto visivo;
  \item \textbf{Backtester elementare}: strategia long/flat sui dati persistiti, con rendimento cumulato, hit-rate e Sharpe semplificato, nel rispetto del protocollo anti-bias;
  \item \textbf{Seconda fonte di dati}: primo passo compiuto con il collector RSS italiano (Sezione~\ref{sec:mercato-italiano}); gli articoli senza scoring del provider sono il banco di prova naturale del futuro modulo di scoring interno;
  \item \textbf{LLM-as-annotator} per un dataset entity-level nello spirito di FinEntity \parencite{tang-etal-2023-finentity}, con revisione manuale a campione, per abilitare il fine-tuning a costi contenuti.
\end{enumerate}

Il vincolo pratico più stringente---uno storico ampio su cui addestrare e validare---è stato rimosso dal cold store (Sezione~\ref{sec:cold-storage}), che accumula senza limiti di ritenzione il flusso di articoli etichettati dal 2018: fine-tuning e benchmark possono essere pianificati su centinaia di migliaia di esempi.

\section{Sintesi}

Il filo conduttore è la trasformazione del prototipo da strumento di monitoraggio a piattaforma sperimentale: ogni componente applicativa corrisponde a una domanda di ricerca verificabile con i dati già raccolti.
L'elemento di maggiore originalità resta la modellazione della rilevanza informativa come campo dinamico che decade e si diffonde nello spazio semantico---ora dotata di formulazione discreta, soluzione in forma chiusa e garanzie di correttezza dimostrate.
