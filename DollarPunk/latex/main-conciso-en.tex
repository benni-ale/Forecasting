...
\vspace{-0.6em}
\begin{center}
\small Live prototype: \href{https://dollarpunk-26786f215068.herokuapp.com/dashboard}{\textcolor{blue}{dashboard}}
\quad $\vert$ \quad Semantic graph: \href{https://dollarpunk-26786f215068.herokuapp.com/semantic-graph}{\textcolor{blue}{\texttt{/semantic-graph}}}
\end{center}
\vspace{-0.2em}
...
Since the space is known only through a finite number of embeddings, one uses the canonical discrete approximation of $-\nabla^2$, the \emph{graph Laplacian} $L=\operatorname{diag}(d_i)-W$, with $n$ tickers as nodes. Edges are admitted only between companies in the same industry; within each industry, weights $w_{ij}\ge 0$ are given by the cosine similarity between embeddings (thresholded, $W$ symmetric), while cross-industry weights are zero. A live technical preview is available at \url{https://dollarpunk-26786f215068.herokuapp.com/semantic-graph}. The PDE becomes a linear system of ODEs,
\[
\frac{dR}{dt} = -(D\,L + \lambda I)\,R, \qquad R(t)\in\mathbb{R}^n,
\qquad\Longrightarrow\qquad
R(t)=e^{-\lambda t}\,e^{-D t L}\,R(0),
\]
This closed form is the graph diffusion kernel of \textcite{kondor2002diffusion}; diffusion over a data/embedding manifold via the graph Laplacian underlies diffusion maps \parencite{coifman2006diffusion}, and \textcite{chamberlain2021grand} show that graph neural networks are themselves discretizations of such a diffusion PDE---so the interpretable formulation here and the learned GNN baseline are two ends of the same continuum. Sentiment is transported by evolving, with the same operator, the weighted sentiment $V=S\cdot R$ and setting $S_i=V_i/R_i$, so that the relevance that moves carries its own tone rather than inflating the local one. Rewritten node by node, $\frac{dR_i}{dt}=D\sum_j w_{ij}(R_j-R_i)-\lambda R_i$, the formulation enjoys three provable guarantees, essential for a system that generates operational signals:
